...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(statusCode.NOT_FOUND).send(fail(statusCode.NOT_FOUND, responseMessage.NO_USER));
    res.status(statusCode.OK).send(success(statusCode.OK, responseMessage.READ_ONE_USER_SUCCESS, user));
  } catch (error) {
